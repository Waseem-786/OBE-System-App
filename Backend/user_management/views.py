...
#     permission_classes = [IsSuperUser, IsUniversityAdmin, IsCampusAdmin, IsDepartmentAdmin]
#     authentication_classes = [JWTStatelessUserAuthentication]

#     def get(self, request, group):
#         university_name = request.query_params.get('university')
#         if not university_name:
#             return Response({"message": "University parameter is required."}, status=status.HTTP_400_BAD_REQUEST)
        
#         # Fetch the university object based on the university_name
#         university = get_object_or_404(University, name=university_name)

#         group = get_object_or_404(CustomGroup,name=group,university=university)
#         if group:
#             users_in_group = CustomUser.objects.filter(groups__name=group)
#             serialized_data = CustomUserSerializer(users_in_group, many=True)
#             return Response(serialized_data.data, status=status.HTTP_200_OK)
#         else:
#             return Response({"message":f"{group} Group not exist"},status=status.HTTP_400_BAD_REQUEST)

#     def post(self, request, group):
#         required_fields = ['username','university']
#         if not all(field in request.data for field in required_fields):
#             errors = {field: ["This field is required."] for field in required_fields}
#             return Response(errors, status=status.HTTP_400_BAD_REQUEST)
...
#         user_name = request.data['username']
#         user = get_object_or_404(CustomUser, username=user_name, university=university)
        
#         # Check if the user is in any group
#         if user.groups.exists():
#             user_group_ids = user.groups.values_list('id', flat=True)  # Get the IDs of groups the user belongs to
#             if group.id in user_group_ids:
#                 return Response({"message": f"User {user_name} is already in {group} group."},
